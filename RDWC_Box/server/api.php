...
/**
 * RDWC Box API
 * Handles data from ESP32 and provides REST endpoints
 */
...
if ($action === 'logs') {
    ...
    $stmt = $pdo->query("SELECT * FROM logs WHERE device_id = '$device_id' ORDER BY created_at DESC LIMIT $limit");
    echo json_encode($stmt->fetchAll(PDO::FETCH_ASSOC));
    exit;
}

// Data endpoint - receive sensor data from ESP32
if ($action === 'data') {
    $device_id = $_GET['device_id'] ?? 'wegabox';
    ...
    // Insert reading
    $sql = "INSERT INTO readings (device_id, root_temp, air_temp, cpu_temp, air_hum, air_press, ph, ph_mv, ec, co2, tvoc, distance, light, ntc, hall, rssi, heap, uptime) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    // Get calibration data to send back
    ...
    if ($pumps) {
        $response['pumps'] = [
            'pump1' => (bool)$pumps['pump1'],
            'pump2' => (bool)$pumps['pump2'],
            'pump3' => (bool)$pumps['pump3'],
            'pump4' => (bool)$pumps['pump4']
        ];
    }
